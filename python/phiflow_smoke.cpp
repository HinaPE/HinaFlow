...
void HinaFlow::Python::PhiFlowSmoke::CreateScalarField(const std::string& name, const int resolution, const UT_Vector3i& size, const UT_Vector3& center, const Extrapolation& extrapolation, const float init_value)
{
...
)", center[0], center[1], center[2], size[0], size[1], size[2], resolution, resolution, resolution, name.c_str(), extrapolation_str.c_str());
    PYrunPythonStatementsAndExpectNoErrors(expr.buffer());

    RECORD_EXPRESSION(expr)
}

void HinaFlow::Python::PhiFlowSmoke::CreateVectorField(const std::string& name, const int resolution, const UT_Vector3& size, const UT_Vector3& center, const Extrapolation& extrapolation, const float init_value)
{
    UT_WorkBuffer expr;
    expr.sprintf(R"(
center = (%f, %f, %f)
size = (%d, %d, %d)
res = (%d, %d, %d)
%s = StaggeredGrid(values=0, boundary=extrapolation.ZERO_GRADIENT, bounds=Box(x=(-size[0]/2.0 + center[0], size[0]/2.0 + center[0]), y=(-size[1]/2.0 + center[1], size[1]/2.0 + center[1]), z=(-size[2]/2.0 + center[2], size[2]/2.0 + center[2])), resolution=spatial(x=res[0], y=res[1], z=res[2]))
)", center[0], center[1], center[2], size[0], size[1], size[2], resolution, resolution, resolution, name.c_str());
    PYrunPythonStatementsAndExpectNoErrors(expr.buffer());
